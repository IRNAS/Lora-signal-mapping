...
void onReceive(int packetSize) {
  ...
  if (data[0] == 'a' && data[10] == 'o' && data[20] == 't') {
    ...
    for (int i = 1; i < 10; i++)           {
      lat_char[i - 1]   = data[i];  // write it into the local arrays
    }
    for (int i = 11; i < 20; i++)          {
      lon_char[i - 11]  = data[i];  // write it into the local arrays
    }
    for (int i = 21; i < 30; i++)          {
      alti_char[i - 21] = data[i];  // write into the local arrays
    }
    for (int i = 32; i < packetSize; i++)  {
      speed_char[i - 32] = data[i];
    }
